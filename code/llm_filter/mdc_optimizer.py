...
def get_optimizer_grouped_parameters(cfg, model, print_fn=print):
    ...
    params_dict_no_decay = {
        name: param for name, param in param_dict.items() if len(param.shape) == 1
    }
    params_dict_decay = {
        name: param for name, param in param_dict.items() if len(param.shape) != 1
    }
    ...
    params_dict_lora_a = {
        name: param for name, param in params_dict_decay.items() if "lora_A" in name
    }
    params_dict_lora_b = {
        name: param for name, param in params_dict_decay.items() if "lora_B" in name
    }
    params_dict_embed_tokens = {
        name: param
        for name, param in params_dict_decay.items()
        if "embed_tokens" in name
    }
    params_dict_remaining = {
        name: param
        for name, param in params_dict_decay.items()
        if not any(x in name for x in ["lora_A", "lora_B", "embed_tokens"])
    }
    ...
    def print_param_group_info(group, group_name):
        ...
        print_fn(
            f"{group_name}: # params: {n_params}M | Sample keys: {list(group.keys())[:2]}"
        )
    ...
    optim_groups = [
        {
            "params": list(params_dict_no_decay.values()),
            "lr": cfg.optimizer.lr,
            "weight_decay": 0.0,
        },
        {
            "params": list(params_dict_lora_a.values()),
            "lr": cfg.optimizer.lr_lora_a,
            "weight_decay": wd,
        },
        {
            "params": list(params_dict_lora_b.values()),
            "lr": cfg.optimizer.lr_lora_b,
            "weight_decay": wd,
        },
        {
            "params": list(params_dict_embed_tokens.values()),
            "lr": cfg.optimizer.lr_embed_tokens,
            "weight_decay": wd,
        },
        {
...
def get_optimizer(cfg, model, print_fn=print):
    ...
    assert cfg.optimizer.name in _optimizers, (
        f"Optimizer {cfg.optimizer.name} not supported"
    )
